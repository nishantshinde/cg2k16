        ...
        for(int r=0;r<input1;r++) {
        	for(int c=0;c<input2;c++) {
        		if(field[r][c]==1) { // destroyed
        			for(int rs=0;rs<=input1/3;rs++) { // steps row
        				for(int cs=-(input2/3);cs<=input2/3;cs++) { // steps column
        					if(rs==0&&cs==0){
        						continue;
        					}
        					length = getLength(field, r, c, rs ,cs);
//        		        	System.out.println(r + " " + c +" " + rs +" " + cs + " - " + length);
        					if(length>maxLength) {
        						maxLength = length;
	...
	private static int getLength(int[][] field, int r, int c, int rs, int cs) {
		int indexr = r+rs;
		int indexc = c+cs;
		int length = 1;
